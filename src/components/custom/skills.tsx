import { FaPython, FaJava, FaReact, FaDocker, FaGitAlt, FaFigma } from "react-icons/fa";
import { SiPostgresql, SiTypescript, SiExpress, SiDrizzle, SiShadcnui, SiPandas, SiGeopandas, SiFolium, SiMermaid, SiScikitlearn, SiNetlify, SiGithubactions, SiJupyter, SiPycharm, SiIntellijidea, SiEclipseide, SiCytoscapedotjs, SiTailwindcss } from "react-icons/si";
...
const skillsData = [
  {
    ...
    skills: [
      ...
      { icon: <IoLogoCss3 />, url: "https://www.w3.org/Style/CSS/Overview.en.html" },
  ...
  },
  {
    ...
    skills: [
      ...
      { icon: <SiFolium />, url: "https://python-visualization.github.io/folium/latest/" },
...
export function Skills() {
    return (
      <div className="text-center text-white grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-6">
        {skillsData.map((category, index) => (
          <Card key={index} className="text-white shadow-md p-4" style={{ backgroundColor: colors[index % colors.length] }}>
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-center">{category.title}</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-3 gap-6 p-4 justify-center items-center">
              {category.skills.map((skill, idx) => (
                <LinkPreview key={idx} className="flex items-center justify-center relative" url={skill.url}>
                  <div className="text-white text-5xl sm:text-6xl md:text-7xl">{skill.icon}</div>
                </LinkPreview>
              ))}
            </CardContent>
          </Card>
